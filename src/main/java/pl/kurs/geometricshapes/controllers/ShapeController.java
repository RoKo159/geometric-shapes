...
public class ShapeController {
    ...
    private SquareManagementServices squareManagementServices;
    private RectangleManagementServices rectangleManagementServices;
    private CircleManagementServices circleManagementServices;
    private ModelMapper modelMapper;


    public ShapeController(SquareManagementServices squareManagementServices, RectangleManagementServices rectangleManagementServices, CircleManagementServices circleManagementServices, ModelMapper modelMapper) {
        this.squareManagementServices = squareManagementServices;
        this.rectangleManagementServices = rectangleManagementServices;
        this.circleManagementServices = circleManagementServices;
        this.modelMapper = modelMapper;
    }

    @PostMapping
    public ResponseEntity<ShapesDto> createShape(@RequestBody CreateShapeCommand shapeCommand) {
        ShapesDto shapeDto = null;
        ShapeType shapeType = shapeCommand.getType();
        if (shapeType == ShapeType.SQUARE) {
            Square squareForSave = modelMapper.map(shapeCommand, Square.class);
            squareForSave.setCreatedAt(LocalDate.now());
            squareForSave.setLastModifiedAt(LocalDate.now());
            squareManagementServices.add(squareForSave);
            shapeDto = modelMapper.map(squareForSave, SquareDto.class);
        } else if (shapeType == ShapeType.RECTANGLE) {
            Rectangle rectangleForSave = modelMapper.map(shapeCommand, Rectangle.class);
            rectangleForSave.setCreatedAt(LocalDate.now());
            rectangleForSave.setLastModifiedAt(LocalDate.now());
            rectangleManagementServices.add(rectangleForSave);
            shapeDto = modelMapper.map(rectangleForSave, RectangleDto.class);
        } else if (shapeType == ShapeType.CIRCLE) {
            Circle circleForSave = modelMapper.map(shapeCommand, Circle.class);
            circleForSave.setCreatedAt(LocalDate.now());
            circleForSave.setLastModifiedAt(LocalDate.now());
            circleManagementServices.add(circleForSave);
            shapeDto = modelMapper.map(circleForSave, CircleDto.class);
        }
